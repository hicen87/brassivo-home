...
test("EPS page is a private login shell with no embedded portfolio data", () => {
  const html = read("eps/index.html");

  assert.match(html, /<link rel="canonical" href="https:\/\/brassivo\.com\/eps\/">/);
  assert.match(html, /<meta name="robots" content="index,follow">/);
  for (const id of ["login-panel", "login-form", "member-email", "member-password", "dashboard", "loading-state", "error-state", "empty-state", "stock-list", "stock-search", "market-filter", "logout-button"]) {
...
test("EPS app authenticates and loads data with credentialed no-store requests", () => {
  const app = read("eps/app.js");

  assert.match(app, /https:\/\/api\.brassivo\.com/);
  assert.match(app, /\/member\/session/);
  assert.match(app, /\/member\/logout/);
...
test("homepage and public indexes link to the private EPS module", () => {
